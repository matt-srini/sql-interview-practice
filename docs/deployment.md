...
**Scope caveat (2026-06-26 audit).** That snapshot was *availability/concurrency*-scoped — it measured that the system stays responsive under load, against a zero-5xx oracle. It did **not** cover execution-*correctness* (a sandbox failure that returns HTTP 200 with an `{error}` body is invisible to a 5xx oracle — exactly how the OpenBLAS RLIMIT_AS abort and other harness failures hid) or sandbox *security*. A later audit found and fixed: the OpenBLAS abort on the pandas/statistics paths (BLAS thread pins), guard-escape file-read vectors (defense-in-depth hardening; Landlock pending — see [`docs/specs/sandbox-threat-model.md`](specs/sandbox-threat-model.md)), and the concurrency-cap inflation below. The load harness now carries a **correctness oracle** and pandas/statistics journeys, and CI carries a deterministic OpenBLAS repro (`tests/test_sandbox_openblas_pin.py`). So "no blockers" held for the failure modes it tested; treat it as availability-ready, with execution-correctness + security tracked separately.
...
**Per-replica sandbox memory budget.** Peak sandbox memory ≈ `MAX_CONCURRENT_EXECUTIONS × RLIMIT_AS (512 MB)` + the app's base RSS (catalog in DuckDB + Postgres pool + uvicorn, ~300–500 MB). Size the Railway container RAM cap from that — e.g. 6 concurrent × 512 MB + 500 MB ≈ a **3.5 GB floor**. The `MAX_CONCURRENT_EXECUTIONS` **default** is `effective_cpu_count() − 2`, where `effective_cpu_count()` (`backend/config.py`) reads the **cgroup CPU quota** (`/sys/fs/cgroup/cpu.max`, v2 → v1 → `os.cpu_count()` fallback), **not** `os.cpu_count()`: inside a container the latter reports the *host* core count, which on a 32/64-core host (with a 2-vCPU slice) would size the semaphore — and thus peak sandbox AS — to **15–31 GB** and OOM the replica. Prod should still **pin `MAX_CONCURRENT_EXECUTIONS` explicitly** as belt-and-suspenders; the cgroup-aware default makes a forgotten pin *safe* rather than catastrophic. (The same `effective_cpu_count()` backs the `MAX_CONCURRENT_HASHES` default.)
...
**Execution-correctness & sandbox-resource carry-forward (2026-06-26 audit).** Every tier must also preserve two properties the original roadmap was silent on. (1) **Execution correctness** — any load/CI check must assert the response *body* is correct, not merely non-5xx (the `backend/loadtest/` correctness oracle + `tests/test_sandbox_openblas_pin.py`); a sandbox failure returns HTTP 200 with an `{error}` body, so a status-code oracle scores it healthy. (2) **Sandbox-resource sizing** — the cgroup-aware concurrency default + the per-replica RAM formula above. At **Tier 3** (externalized execution) specifically: the worker fleet must export the `_sandbox_env` pins (incl. `OPENBLAS_NUM_THREADS=1`) regardless of worker host core count, apply `RLIMIT_AS` per-sandbox (not per-worker, so one bad sandbox can't kill the worker), and report `{correct, error}` back to the web tier so a silent worker failure cannot surface as a healthy 202/200.
...
The **canonical sandbox threat model** — every defense layer, what it does and does not contain, and the known residuals (the file-read introspection tail, closed by pending Landlock) — is [`docs/specs/sandbox-threat-model.md`](specs/sandbox-threat-model.md). This section covers the **OS-level / Railway-equivalent** layers specifically: the platform denies us the usual container primitives (a custom seccomp profile, `--read-only`, `NET_ADMIN`), so we implement equivalents in code / in the image.
...
| **Native-math thread pinning** | `_sandbox_env` exports `OPENBLAS_NUM_THREADS`/`OMP_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMEXPR_NUM_THREADS`=`1` into the sandbox subprocess. numpy's bundled OpenBLAS otherwise sizes a per-core scratch-buffer pool to the (container-over-reported) host core count, whose *virtual* address-space reservation can exceed the 512 MB `RLIMIT_AS` applied before numpy import, aborting the process ("Memory allocation still failed after 10 retries, giving up"). One thread per sandbox fits comfortably and is deterministic; throughput comes from process-level concurrency (`MAX_CONCURRENT_EXECUTIONS`). Only the numpy-importing paths (Pandas, Statistics-numerical) are affected. | ✅ active |
| **Filesystem read-scope (Landlock)** | `landlock_sandbox.py` applies a per-execution **Landlock** ruleset (Linux ≥5.13; raw `ctypes`, no dependency) in the harness, allowing read+exec only on the Python runtime / system libs / datasets / `/tmp` and denying everything else — so a guard escape that reaches `os` still cannot read `backend/content/**` (the answer keys) or app source, which also cuts the response-body exfil channel. **Fails open**: the app logs availability at boot (`Sandbox Landlock FS read-scope: available (ABI vN)` / `... UNAVAILABLE ...`) — a WARNING in prod means Railway's host kernel lacks it. CI-validated (`tests/test_sandbox_landlock.py`). | ✅ active (Linux) · ⚠️ confirm on prod kernel via boot log |
