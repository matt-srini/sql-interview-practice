...
- **(MCQ tracks) The dilution trap — a green balance validator does NOT mean your batch is clean.** `_validate_answer_position_balance` / `_validate_answer_length_balance` measure the *whole* group and only ERROR above the ceiling (40% position / 55% length). A freshly-authored batch can be **100% biased and still pass**, because the surrounding pre-existing questions average it down under the ceiling — and a biased **chain** sub-pool likewise hides, diluted by standalone questions, inside the combined `mock` group. Validator-green is not evidence your new questions are balanced. Two countermeasures (added 2026-06-08):
  - **(1) Draw-surface split.** The validators now group the mock pool by *what a session actually draws from*: `mock-standalone` (Benchmark + Custom/Drill) vs `mock-chain` (Interview Loop), because a session only ever draws from one and a biased chain pool is gameable in the Loop regardless of standalone dilution. `mock-chain` is eased in at **WARN-level** (promote to ERROR after the chain-debiasing pass clears the existing chain pools — e.g. `data-engineering/hard/mock-chain` at 65% unique-longest); `mock-standalone`, `practice`, and `sample` keep **ERROR** teeth.
  - **(2) Batch check.** Run `scripts/check_batch_balance.py <ids>` on the questions you just authored/edited; it checks *their own* distribution against the stricter **authoring targets** (≤40% position, ≤45% unique-longest) and FAILs if biased — catching exactly what the group validator launders away. *Precedent: a 2026-06 stats-hard chain batch was 15/15 "correct is longest" yet passed the group validator at 53.2% by dilution; the chain pool feeding Interview Loop was ~100% gameable while CI was green.*
...
|---|---|---|---|---|
| SQL | 118 | 196† | **1.66×** | executable analytics; †28 chain questions (10 new Interview Loop chains) added 2026-06-20 lift the ratio above the 1.0–1.5× band — a deliberate **chain-only exception** (Elite Interview Loop only, never benchmarks/drills); standalone budget unchanged at ~1.30× (153 standalone). See [`DECISIONS.md`](./decisions/DECISIONS.md) 2026-06-20, + 3 SQL-medium escalation follow-ups (2026-06-22) |
| Python | 81 | 103 | **1.27×** | executable algorithms |
| Pandas | 93 | 132 | **1.42×** | executable analytics |
