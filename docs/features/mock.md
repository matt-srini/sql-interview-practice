...
Benchmark composition follows track-specific type targets where the bank supports them. PySpark benchmarks target code-adjacent Spark forms; Statistics benchmarks enforce `1 numerical + 2 conceptual`; ML Fundamentals and Experimentation benchmarks explicitly pull from `scenario`, `conceptual`, `predict_output`, and `debug` types. Single-track Data Modeling benchmarks use a `["scenario", "conceptual", "scenario", "conceptual", "scenario"]` type sequence (3 scenario + 2 conceptual, no debug) at medium and hard; at easy difficulty the bank holds only 1 scenario question, so the declared type sequence degrades to `1 scenario + 4 conceptual` — the easiest-available questions fill the remaining slots (`type_fallback: true` applies; see degradation contracts below). Mixed benchmark slots draw from the named tracks with fresh-first per-track selection; if one track's easy pool is exhausted for the user, the slot falls back to the deepest-available other track in the role profile (`track_substituted: true` flag set in response). **Important:** Mixed benchmark per-track slots (including the Analytics Engineer Data Modeling slot) are NOT filtered by question type — they draw from the full difficulty-band pool for that track, which may include `debug` questions. The single-track type-targeting sequence (`_benchmark_type_targets`) is applied only when a single track is selected; it is bypassed in the Mixed benchmark path where each track fills its slot count via fresh-first pool sampling.
...
`type_fallback` is the expected steady-state for Data Modeling easy benchmarks (1 scenario in the bank vs. 3 declared) and may occur on any track when the available mock pool is thin at a given difficulty. It does not indicate an error; it is a first-class composition outcome.
...
1. **Discard within the 2-minute window** — `DELETE /api/mock/:id` called within 120 s of `started_at` deletes the session row, which naturally reverts all quota counters (weekly benchmark count, daily benchmark count, daily custom count). Misclicks and "wrong track" recoveries don't penalise the user.
