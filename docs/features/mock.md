...
| `pool_exhausted: true` | Interview Loop: no unconsumed chains remain for user × track × difficulty | 409 returned by default. The access map carries `replayable: true`, and the UI offers a **consent-gated Replay** (`POST /start` with `replay: true`) that re-draws a completed chain and sets `is_replay: true` on the session. No *silent* fallback — re-showing chains requires the explicit Replay action. See § Follow-up Chain Atomicity. |
...
- `mode=interview_loop`: Free/Pro → `plan_locked`; Elite → **per-difficulty chain-availability check**. Interview Loop requires the user to choose **medium or hard** (easy is never offered — no chains exist at easy difficulty). The access endpoint returns a per-difficulty map `access[medium|hard]`, each entry carrying `{ can_start, block_reason, escalates }`. Difficulties with no chains in the bank return `can_start: false, block_reason: "no_chains"` (the UI greys these per the standard grey+message gating convention — see [DECISIONS 2026-06-16](../decisions/DECISIONS.md)). `escalates: true` is set for Python-medium and Data-Modeling-medium (their chains escalate to hard follow-ups); the UI displays these as "medium → hard". Track-scoped availability rules: **Python → medium only** (hard has no chains); **ML Fundamentals and Experimentation → hard only** (no medium chains); **SQL, Pandas, PySpark, Data Engineering, Data Modeling, Statistics → medium and hard** (Data-Modeling-medium escalates). Availability is derived from the question files (`backend/routers/mock.py` `_chain_parents_for` / `_interview_loop_access`), never hardcoded in the UI. The Start button is gated on the selected difficulty's `can_start`. When a difficulty entry is `pool_exhausted` (user consumed all chains at that difficulty), the entry returns `can_start: false, block_reason: "pool_exhausted", replayable: true` and the session-brief rail shows a positive completion message — *"You've completed every {difficulty} Interview Loop chain for this track — nice work…"* — plus a **Replay** button for consent-gated re-draw (see § Follow-up Chain Atomicity rule 6). *(Supersedes the 2026-06-15 "no difficulty selector" decision — see [DECISIONS 2026-06-16](../decisions/DECISIONS.md); replay added [2026-06-19](../decisions/DECISIONS.md).)*
...
6. **Pool exhaustion → consent-gated replay:** when no unconsumed chains remain for a user × track × difficulty, `POST /start` returns **409** with `pool_exhausted: true` and `replayable: true` *by default*. The exhausted state is framed as a completion (not a dead-end): the UI shows a positive "you've completed every chain" message, the in-panel track/difficulty/mode selectors as next steps, and a **Replay** button. The Replay button re-issues `POST /start` with `replay: true`; `_select_chain` then re-draws a completed chain (fresh-first among the consumed set) and returns `is_replay: true`, which surfaces an in-session "you're revisiting a completed chain" notice. **No *silent* fallback** — a repeat only ever happens on the explicit Replay action. (Supersedes the former hard "no soft fallback / dilutes the readiness signal" lockout — the readiness *score* is a separate aggregate that Interview Loop doesn't feed, and the 30-pt mock booster it does feed already counts recycled benchmark/custom repeats; see [DECISIONS 2026-06-19](../decisions/DECISIONS.md).)
...
7. If no eligible chains exist, the failure is split by cause:
   ...
   - **Chains existed but this user consumed them all at this difficulty** (dynamic): `_select_chain` returns **409** with `pool_exhausted: true` and `replayable: true` *unless* `replay: true` was passed. The access map surfaces this as `access[difficulty].pool_exhausted` + `replayable`, so the UI disables the primary Start, frames the state as a completion, and offers a **Replay** button (which re-issues `/start` with `replay: true` — see Selection rule 6).
