...
| Section | Description |
|---|---|
| **Track Overview** | One card per active track (all 9 tracks). Each shows solved/total, an animated progress bar, median solve time, accuracy %, easy/medium/hard breakdown, and (Elite) a per-track readiness score badge. On mobile (≤600px) the row reflows to two lines — name + readiness chip on the first, progress bar + count + accuracy on the second — dropping the tagline, median solve time, and the "Mock to level up" nudge to stay compact. Data from `/api/dashboard` + `/api/dashboard/insights`. |
