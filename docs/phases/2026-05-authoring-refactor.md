...
### Phase 2 — Content alignment 🟢 SQL complete (passed re-audit after remediation); 🟢 Python COMPLETE 2026-05-23 (commits cd30279, d75fee6, de0d298, 1bddfac) — taxonomy 16→19 families, retag 75 mock Qs, delete 23037/23044/23068, fix chain validator, revert python.md regressions, generator library + expansion layer, 47 oversized test cases converted/removed (98 MB → 2.5 MB); all A1–A6 acceptance criteria PASS; 🟢 PySpark COMPLETE 2026-05-24 (commits a6164ed, fc160df, c194d32, e213b54, 035f8f2, 054fb77, e8875ec, a554dcd, 99bb06f, 08dcc93, 329ae1c) — taxonomy 21→23 families, 277→940 resolved tags / 0 unresolved, 42 over-tags stripped, 12 new practice Qs, 5 mock-only Qs retagged, 129 new mock-only Qs (21→150 total), footprint 116→128 practice; 🟢 Pandas COMPLETE 2026-05-24 (commits c261d41, d6a236d, ec805ef, eae8550 + hygiene) — 21-family registry locked, 6 ⚡ families classified (3 practice-grounded / 3 mock-only realism), 21 chains + 56 standalones authored (26→110 mock-only), all hints green, validator passing; 🟢 Data Engineering COMPLETE 2026-05-24 — 21-family registry locked (no realism families; all gradeable as MCQ), 5 tag remaps, 16 chains + 80 standalones authored (14→110 mock-only, 1.21× ratio), all hints green (H1 leak ban enforced), validator passing, track added to _TAXONOMY_VALIDATED_TRACKS; other tracks pending (pre-existing ml-fundamentals validator failures — 10 single-concept questions — surfaced after B1 fix; out of PySpark scope, filed as ml-fundamentals Phase 2 prerequisite); 🟢 Data Modeling COMPLETE 2026-05-25 — 22-family registry locked (no realism families; all gradeable as MCQ), 22 orphan tag instances remediated across 20 questions + 2 path files, 10 chains + 65 standalones authored (13→96 mock-only, 1.20× ratio), all hints green (H1 leak ban enforced), validator passing, track added to _TAXONOMY_VALIDATED_TRACKS; 🟢 Statistics COMPLETE 2026-05-26 — 13-family registry locked (no realism families; dual-subtype MCQ + numerical Python), concept_families.py extended (errors & power + variance decomposition & ANOVA patterns + survival analysis family), path focus_concepts lowercased, 116 mock-only authored (0e/66m/50h, 1.16× ratio), all hints green (H1 leak ban enforced), validator passing, track added to _TAXONOMY_VALIDATED_TRACKS; ML Fundamentals and Experimentation pending
...
The locked semantics, applying to all track audits running 2026-05-24 onward (Statistics, ML Fundamentals, Experimentation):
...
| 2026-05-24 | **Pandas Phase 2 doc-hygiene closeout (H1–H5) COMPLETE.** Sonnet | Pandas Phase 2 CLOSED. Durable rules survive without this tracker. **H1:** `docs/concept-taxonomy.md` top-of-file ⚡ callout updated from "Pandas Phase 2 in progress" to "Pandas Phase 2 complete"; the six Pandas family headers had `*(practice-grounded — Phase 2)*` / `*(mock-only realism — Phase 2)*` suffixes stripped — now read as normal registered families; mock-only realism designation stays in body prose. **H2:** `docs/tracks/pandas.md` family count sentence updated to "21 families (Phase 2 complete) … three practice-grounded, three mock-only realism lenses"; classification table header changed from "Phase 2 (2026-05) classification" to "2026-05 classification (locked)"; grounding references updated to remove "Phase 2 adds" phrasing. **H3:** `MOCK_ONLY_REALISM_FAMILIES["python-data"]` already set correctly in `backend/concept_families.py` (3 families: METRIC INTERPRETATION & DENOMINATOR CHOICE, OUTPUT SANITY VALIDATION, PERFORMANCE-AWARE ANALYTICS) — verified, no change needed. **H4:** `docs/content-authoring.md` mock-only add-on updated 523 → 607. **H5:** CLAUDE.md Pandas row updated (26 → 110 mock-only; table updated 36+12 → 36+50, 23+14 → 23+60); mock-only totals updated 523 → 607; Phase 2 status line updated to 🟢 Pandas COMPLETE. **No outstanding items.** Pandas Phase 2 = CLOSED. |
| 2026-05-25 | **Data Modeling Phase 2 doc-hygiene closeout (H1–H7) COMPLETE.** Sonnet | Data Modeling Phase 2 CLOSED. Durable rules survive without this tracker. **H1 (orphan remap):** 22 orphan tag instances remediated across 20 questions + 2 path files — 5 orphan tag strings removed (`STORAGE ARCHITECTURE TRADEOFFS`, `PARTITIONING & CLUSTERING`, `DATA MODELING GOVERNANCE`, `TEMPORAL QUERY`, `WINDOW FUNCTIONS`), instances replaced with nearest registered families (`DENORMALIZATION TRADEOFF`, `AGGREGATE & SUMMARY DESIGN`, `DIMENSIONAL MODELING`, `WIDE VS NARROW`); orphan resolver `unresolved=0` confirmed. **H2 (validator enable):** `"data-modeling"` added to `_TAXONOMY_VALIDATED_TRACKS` in `backend/scripts/validate_content.py` with comment; `validate_content.py` passes clean. **H3 (concept-taxonomy.md):** preamble updated — DM removed from pending-track list, DM Phase 2 complete bullet added (22 families, no realism families, MCQ-gradeable, `MOCK_ONLY_REALISM_FAMILIES["data-modeling"] = set()` enforced); question types updated to include `debug (mock-only only)`; tag inventory refreshed (44 unique / 268 occurrences → 41 unique / 515 occurrences post-remap + 96 mock-only added). **H4 (data-modeling.md Coverage section):** "Coverage & sizing targets" section added after Verification (80 practice / 96 mock-only / 1.20× / 10 chains / type distribution / all 7 follow-up dimensions covered). **H5 (concept_families.py):** `MOCK_ONLY_REALISM_FAMILIES["data-modeling"] = set()` added with design-rationale comment immediately after `"data-engineering"` entry. **H6 (content-authoring.md):** DM row added to "Established precedent" table (80 / 96 / 1.20× / constructed reasoning MCQ); DM added to "Closed tracks" list; Statistics / ML Fundamentals / Experimentation remain pending. **H7 (phases tracker):** Phase 2 status heading updated with 🟢 DM COMPLETE segment; band-semantics list updated (DM removed from pending); closed-tracks note updated; this row records the closeout. **No outstanding items.** Data Modeling Phase 2 = CLOSED. |
| 2026-05-26 | **Statistics Phase 2 Stage B + H-series closeout COMPLETE.** Sonnet | Statistics Phase 2 CLOSED. Durable rules survive without this tracker. **Stage B (mock-only authoring):** 116 mock-only questions authored (0e / 66m / 50h) across 13 concept families, ratio 1.16× — inside the 1.15–1.25× target band. Questions span 13 ITEMs covering all families (hypothesis testing, confidence intervals, sampling & CLT, correlation/regression/causality, experimental design, Bayesian inference, errors & power, probability & combinatorics, distributions, multiple testing & correction, descriptive statistics, variance decomposition & ANOVA, survival analysis & time-to-event). Chains authored: CRR-2 (72052→72054), CRR-3 (72055→72057), EXP-1 (72060→72062), BAY-1 (72066→72068), CI-1 (72071→72072), CI-2 (72073→72075), EPow-1 (72076→72078), MT-1 (72096→72098); CRR-1 retroactively wired (73028→73032→73040). **H1 (orphan resolver):** 0 orphans across 216 questions — all concept tags resolve to a registered family. **H2 (validator enable):** `"statistics"` added to `_TAXONOMY_VALIDATED_TRACKS` with comment. **H3 (concept-taxonomy.md):** 0 ⚡ markers in Statistics section — confirmed. **H4 (statistics.md Coverage section):** "Coverage & sizing targets" section added (100 practice / 116 mock-only / 1.16× / 13 families / all ≥4 mock-only). **H5 (concept_families.py MOCK_ONLY_REALISM_FAMILIES):** already set `set()` in prior session — confirmed no change needed. **H6 (CLAUDE.md + content-authoring.md):** Statistics row updated (97→100 practice, 8→116 mock-only); totals updated (865→868 practice, 796→904 mock-only); content-authoring.md validator-coverage table updated (⛔→✅), question-bank table and totals updated. **H7 (tracker):** Phase 2 status line updated with 🟢 Statistics COMPLETE segment; this row records the closeout. **Infra fixes during closeout:** `concept_families.py` — `"errors & power"` added as self-resolving pattern to its own family; `"sum of squares"`, `"between-group"`, `"within-group"`, `"explained variance"`, `"eta squared"` added to variance decomposition & ANOVA patterns; `applied-stats.json` and `stats-for-analysts.json` path `focus_concepts` lowercased to valid family tags; 73070 first hint rewritten to remove p-value leak; 15 questions with `"errors & power"` concept tag patched (was using family name before pattern was registered). **No outstanding items.** Statistics Phase 2 = CLOSED. |
| 2026-05-26 | **Statistics Phase 2 Stage C audit remediation COMPLETE.** Sonnet | Statistics Phase 2 audit remediations applied. **C2 fix (Bayesian-frequentist co-tag violation):** `"frequentist inference"` removed from Q72017 (medium) concepts → `["Bayesian inference", "probability"]`; `"frequentist inference"` removed from Q73033 (hard) concepts → `["Bayesian inference", "prior information", "decision making under uncertainty"]`. **C4/E2 fix (resolver substring shadow):** bare `"variance"` pattern removed from `descriptive statistics` family in `concept_families.py` (was shadowing `variance decomposition`, `variance reduction`, `bias-variance tradeoff`); `"bias-variance tradeoff"` and `"bias variance"` patterns added to `correlation, regression & causality` with rationale comment; resolver spot-check 8/8 pass, validator 0 orphans. **Answer key corrections:** Q72017 correct_option 2→1 (frequentist-objects-because-fixed-constants is the correct answer; explanation rewritten to explicitly refute each distractor); Q73018 correct_option 2→1 (simple-model MSE=15 < complex-model MSE=19 is the correct answer; explanation rewritten); Q73020 correct_option 2→1 (Beta(10,5) conjugate posterior is correct; explanation rewritten with conjugate update derivation and distractor refutations). **H6 fix:** Statistics row added to precedent table in `docs/content-authoring.md` (100 / 116 / 1.16× / hybrid); Statistics added to "Closed tracks" list; ML Fundamentals / Experimentation remain pending. **H3 fix:** `docs/concept-taxonomy.md` ⚡ callout updated — "Statistics Phase 2 complete" stanza added (13 families, lowercase canonical, no realism families by design, dual-subtype rationale, `MOCK_ONLY_REALISM_FAMILIES["statistics"] = set()` enforced); Statistics removed from "other tracks pending" sentence. **H4 fix:** `docs/tracks/statistics.md` "Concept families" section: "12 families" → "13 families"; "Coverage & sizing targets" section extended with target-ratio justification (1.15–1.25× band, 1.16× landing, PySpark precedent anchor), per-subtype ratio table (conceptual 1.42× from 60→85 / numerical 0.78× from 40→31, 0.78× materially below 1.0× acknowledged and flagged as provisional — first remediation candidate if exhaustion observed), mock difficulty split defence (66:50 = 1:0.76, explained by family-pair recombination ceiling at hard and 0-easy-mock design constraint). **I2-b resolution:** path edit `backend/content/paths/experimental-design-inference.json` committed in `dc218f3` confirmed justified — Q72026 (Poisson distributions, practice) was correctly removed from the inference path (topic mismatch); UPPERCASE focus_concepts `SEQUENTIAL TESTING`, `METRIC SENSITIVITY`, `GUARDRAIL METRICS` correctly stripped (not Statistics families; belong to Experimentation taxonomy). **Experimentation Phase 2 handoff note:** when authoring Exp Phase 2, verify that `SEQUENTIAL TESTING`, `METRIC SENSITIVITY`, and `GUARDRAIL METRICS` map to Experimentation families (or add families if not present) — they appeared as aspirational Stats path tags and must not float unresolved. |
| 2026-05-26 | **Statistics Phase 2 AUDIT VERDICT: PASS (after two remediation rounds).** Auditor (Opus) | Independent post-execution audit against durable contract docs. **Round 1 (against `ed5f149`): FAIL** with 7 blocking items: (C2) Q72017 and Q73033 carried `frequentist inference` co-tag with `Bayesian inference`, violating Stage A D2-A non-inversion lock; (C4/E2) resolver substring shadow on `variance` mis-routed 11 tags (7 `variance decomposition`, 3 `variance reduction`, 1 `bias-variance tradeoff`) to `descriptive statistics` instead of intended families; pre-existing factual answer-key errors in Q72017 (correct_option=2, "frequentist agrees because both produce same numerical answer" — false), Q73018 (correct_option=2, "lower bias guarantees better generalization" — false), Q73020 (correct_option=2, "Normal posterior because CLT applies" — false; Beta-Binomial conjugate gives Beta); H6 partial (Stats row missing from precedent table, closed-tracks list); H3 partial (Statistics still listed in concept-taxonomy.md pending callout); track-doc 12-vs-13 family inconsistency; I2-b undisclosed scope-creep (`experimental-design-inference.json` edit including Q72026 removal). **Round 2 (against `db46e2a`): FAIL** on residual H4 doc-accuracy — Coverage section per-subtype table reported conceptual 51/72 (1.41×) and numerical 49/44 (0.90×); on-disk actual is conceptual 60/85 (1.42×) and numerical 40/31 (0.78×). Sonnet fabricated subtype counts that did not match the JSON. **Round 3 (this row): self-remediated by auditor (doc-only, no question changes)** — Coverage section per-subtype table corrected to actual on-disk numbers (60c/40n practice, 85c/31n mock; numerical ratio 0.78×); defensive narrative re-cast to acknowledge 0.78× as materially below 1.0× contract floor and flagged as provisional with first-remediation-candidate framing; numerical-runway support figures (≈1 per family at medium, ≈1.4 at hard; ~15 distinct hard benchmarks before fresh-first exhaustion under `1 numerical + 2 conceptual` benchmark spec) computed from on-disk reality. Tracker row 823 per-subtype claim corrected to match. **All other Round 1 items verified remediated against `db46e2a`:** C2 — Bayesian-resolving questions = 17, frequentist co-tag violations = 0 ✓; C4/E2 — all 11 variance-related tags now resolve to semantically correct families (verified via per-tag resolver enumeration) ✓; answer keys corrected with rewritten explanations refuting each distractor ✓; H6 precedent table contains Stats row (100/116/1.16× hybrid) and closed-tracks list updated ✓; H3 callout contains "Statistics Phase 2 complete" stanza, Stats removed from pending ✓; track-doc says 13 families ✓; I2-b acknowledged in tracker with Experimentation handoff note for the 3 stripped platform-mechanic concepts. **Verified clean:** zero `"type": "mcq"` hits; zero orphans (216 questions); validator passes with no Stats-attributable warnings; per-track rule 3 ceiling not breached; 12 chains all live within mock-only with all 7 follow-up dimensions exercised; `MOCK_ONLY_REALISM_FAMILIES["statistics"] = set()` enforced. **Open semantic flags (NOT blocking, file for content-quality pass):** (a) Q73008 "Bias-Variance Tradeoff" carries `variance decomposition` tag → now resolves to ANOVA family but content is bias-variance; should re-tag to `bias-variance tradeoff` to route to `correlation, regression & causality`. (b) Q73023 "Variance of a Linear Combination" carries `variance decomposition` tag → resolves to ANOVA family but content is Var(aX+bY) probability identity, not ANOVA. Re-tag candidate. (c) Pre-existing easy numerical questions (71005, 71015, 71028) are formula-recall, violating the track-doc anti-pattern but predate Phase 2 scope — file as separate framing-debt remediation. (d) Stage A target ratio was not locked as a single number on disk; doc states the 1.15–1.25× band, executor landed at 1.16× (low end); future audits should treat band-landing as the standard. **Procedural:** P1 verified — commits ed5f149, db46e2a use descriptive titles, no self-grading. P2 has an outstanding finding: I2-b scope-creep (path-file edits + pre-existing question hint rewrite at Q73070 + 15 pattern-registration re-resolves) was disclosed in hand-back rather than surfaced before self-applying; closeout-blocking is not an automatic P2 exception. Recommend future runbook update to codify "validator-blocked = closeout-blocking" handoff pattern. **Verdict: Statistics Phase 2 = CLOSED + AUDITED.** Tracker may tick Stats. **Open tracks remaining: ML Fundamentals (Phase 2), Experimentation (Phase 2), Data Modeling (Phase 2.5 re-balance).** |
| 2026-05-25 | **Data Modeling Phase 2 AUDIT VERDICT: PASS.** Auditor (Opus) | Independent post-execution audit against durable contract docs (not the executor's hand-back). **Counts:** 80 practice (25e/30m/25h) + 96 mock-only (0e/45m/51h), ratio **1.20×** — inside the 1.0×–1.5× contract band, matches the Stage A target. Math reconciles across CLAUDE.md footprint+totals, `content-authoring.md` Question-bank table and precedent table, `data-modeling.md` Coverage section, and on-disk JSON. **Terminology:** zero `"type": "mcq"` hits; only `conceptual` / `scenario` / `debug` (with debug locked mock-only-only in `data-modeling.md` line 18 + reflected in `concept-taxonomy.md` line 880). **Orphan tags:** resolver `unresolved=0` across 176 questions / all DM tag slots — confirms `dac5932` cleanly removed `STORAGE ARCHITECTURE TRADEOFFS`, `PARTITIONING & CLUSTERING`, `DATA MODELING GOVERNANCE`, `TEMPORAL QUERY`, `WINDOW FUNCTIONS` and remapped to registered families. **Realism path (ii) codified:** `MOCK_ONLY_REALISM_FAMILIES["data-modeling"] = set()` with rationale comment mirroring the DE precedent (lines after DE entry in `concept_families.py`); `_TAXONOMY_VALIDATED_TRACKS` includes `"data-modeling"` so the absence is now machine-enforced, not silent. **Chains:** 10 parents / 21 follow-ups (4 medium + 6 hard parents); all 7 follow-up dimensions covered (business_rule×5, data_quality×3, performance×3, edge_case×3, ambiguity×3, scale×2, stakeholder×2); no chain spans difficulty-file or practice/mock boundary. **Type-mix:** mock-only is 55% scenario / 29% conceptual / 16% debug — well below the 70% conceptual quality flag; debug type-introduction has real content (15 questions). **C-framing spot-check** (62054, 62065, 63045, 63055, 63070, 63075): each exercises modeling reasoning at its tagged tier — grain decisions, conformed-dimension governance post-M&A, SCD under conflict, semantic-layer extension to DS teams, zero-downtime schema migration, cross-fact double-count debug. No SQL-in-DM cosmetic re-skinning. **Difficulty split (mock m:h):** 45:51 ≈ 1:1.13, hard-skewed within precedent (Pandas/DE both 1:1.20). **Tests:** `tests/test_20_data_modeling.py` 21/21 pass; `validate_content.py` passes clean with DM now inside the taxonomy-validated set. **Procedural:** P1 verified — remediation commit `dac5932` titled `refactor(data-modeling): Phase 2 taxonomy remediation and registry lock` (descriptive, not self-grading). P2 verified — every file touched by `dac5932` maps to a stated H1–H7 work item; two DM-internal path-file edits (`backend/content/paths/dbt-and-modern-analytics-modeling.json`, `wide-tables-and-obt.json`) fell out of the orphan-tag remap scope and do not bend any contract. **No remediation needed. No outstanding items.** Data Modeling Phase 2 = CLOSED + AUDITED. Tracker ticks DM. |
