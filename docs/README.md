...
| [`runbooks/prod-user-stats.md`](runbooks/prod-user-stats.md) | **Prod user / login stats** — read-only snapshot of live registrations, logins, and activation via [`backend/scripts/check_prod_users.py`](../backend/scripts/check_prod_users.py). Answers "how many users have registered / logged in so far?" Reads the prod DB URL from `backend/.env`; anonymous-first identity model explained. |
