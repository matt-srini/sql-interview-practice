...
1. **Hero** — Logged-out: 2-col grid with interview-urgent eyebrow copy, a large headline tying interview readiness to durable on-the-job reasoning, CTAs ("Try a free sample →" / "Find your role ↓"), and `HeroIDE` component (character-by-character SQL typing animation, result rows stream in ~55ms/row). Logged-in: 3-card strip (Resume / Dashboard / Mock) using `.lp-li-card`. The **Resume** card links to `/practice/<topic>?resume=1` for the user's most-recent practice track (`recent_activity[0].topic`, which is practice-only — see [backend.md](backend.md) §`/api/dashboard`) — **not** a hard-coded question id. AppShell resolves `?resume=1` to the track's **next-up** question (the SidebarNav "NEXT"), so Resume continues forward instead of re-opening a question the user already solved. **First paint / no refresh flash:** the whole layout branches on `user`, which is null until `GET /auth/me` resolves — historically that flashed the logged-out marketing hero on reload, then flipped the greeting once `/dashboard` loaded. Now `AuthContext` persists a boolean `dt_authed` hint (localStorage, no PII); a returning user paints a neutral `HeroSkeleton` while auth resolves (anonymous visitors — no hint — still get the marketing hero immediately), and the logged-in hero renders only once its dashboard data is ready, so the greeting appears once, stable. The greeting itself is driven by the `is_new` flag on `/auth/me` (account age < 24h → "Welcome", else "Welcome back") — authoritative and synchronous with `user`, so it never flips (it replaced an earlier `totalSolved`-based guess, which mislabeled a returning-but-never-solved user as new).
...
- `ROLES` (imported from `roleRegistry.js`, the **role→track SoT**) defines the role tab entries with ordered `tracks[]` slugs + tagline. It is the one place to edit the role→track mapping; the mock role selector and dashboard role filter derive from it (see the Interview-prep §Shared pieces below)
...
- **Resume upgrade after sign-in**: `/` is the only upgrade-CTA surface whose layout differs by auth state, so a logged-out visitor who clicks upgrade here, signs in, and returns would otherwise land on the unrelated logged-in home with no trace of what they started. `LandingPage` captures `location.state?.upgradeTier` once at mount (cleared from history the same way the `scrollTo` state is, so refresh/back can't refire it) and, only when `user` is set, renders a "Continuing your upgrade to {tier}…" banner (`.landing-upgrade-resume-banner`) with an `UpgradeButton` in `autoTrigger` mode — see [`pricing.md`](features/pricing.md) § Upgrade flow and [DECISIONS.md](decisions/DECISIONS.md) (2026-07-14).
...
All five legal pages open as **modal overlays** from the landing footer (`<Link to="/…" state={{ backgroundLocation: location }}>`): the modal layers over the landing, which stays mounted at its current scroll position. Every close path — the footer "Close" button, the top-right ✕, or a backdrop click — returns to the footer **exactly where the user was** (`navigate(…, { replace: true, state: { preserveScroll: true } })`), never jumping to the top or redirecting to `/`. This lets a user open several legal links in one sitting without re-scrolling. (A dedicated-page variant with a "Back to home → footer" smooth-scroll was tried first and reverted — a full-page nav unmounts the landing, so returning always flashed top-then-scroll; see `docs/decisions/DECISIONS.md` 2026-06-21.)

`PolicyModal` (App.js) renders the overlay through a **React portal into `document.body`**, not inline in the route tree. The `.route-transition` wrapper carries a `transform` during its enter animation, and a transformed ancestor becomes the containing block for `position: fixed` descendants — which would pin the modal to the document (off-screen when opened while scrolled down) instead of the viewport. Portaling sidesteps that class of bug entirely; React context still flows through the portal, so the modal's inner `<Routes>` keep their router context.

Visited **directly** (a shared `/faq` URL, a crawler), each route renders as a standalone page (minimal topbar, centered card, scroll-to-top on mount, full `<Helmet>` + JSON-LD for FAQ) with a plain "Back to home" link to `/`. Each page component takes an `isModal` prop that switches between the modal and standalone presentations.

`RouteTransition` (App.js) owns scroll-to-top on route changes but skips the reset when a hash anchor is present, when a modal is open over a background page (`state.backgroundLocation`), or when a close asked to keep position (`state.preserveScroll`).

### PricingPage (`/pricing`)

Public, full-detail plan comparison (no auth gate) — the "no hiding behind short adjectives" surface. Renders entirely from a single data source, **`frontend/src/data/tierFeatures.js`** (`COMPARISON_TIERS`, `COMPARISON_GROUPS`, `FOOTNOTES`), so the matrix has one home and tracks the entitlement SoT (`pricing.md` / `mock.md` / `unlock.py`). Layout: tier summary cards (Pro featured, `UpgradeButton` CTAs) → grouped Free/Pro/Elite feature matrix (✓ / — / value per cell, Pro column tinted). Free model: easy only — medium and hard show `—` in the Free column. Reached from a "Compare every feature in detail →" link below the landing `#landing-pricing` grid (`.lp-pricing-compare-link`) and from every in-app "see plans" CTA (TierBanner, practice path-sidebar hint, Account). Those CTAs pass a `returnTo` via router state; when present, `/pricing` renders a contextual "← Back to {label}" link at the top (plain "← Back" without a label), else nothing on a cold/marketing landing (internal-path guarded). See [`pricing.md`](features/pricing.md) § Upgrade CTA reference for the full navigation convention. The internal mirror of the same content is `docs/tier-wise-features.html`.
...
- "Continue where I left off" button → navigates to the next-up question (`pickContinueQuestionId`: next-up `is_next`, else first unlocked).
- **Completed state** (`allAccessibleSolved` = `pickNextUpQuestionId(catalog) === null && totalSolved > 0` — i.e. no actionable `is_next` left): the primary CTA swaps from **Continue →** to **"Unlock more"** (free/pro with locked questions) or **"Explore tracks →"** (otherwise), and a milestone banner appears — 🏁 *solved every accessible question* (free/pro upgrade nudge) or 🏆 *Track complete* (Pro/Elite, everything solved). This is keyed on the **`is_next`-only** `pickNextUpQuestionId` returning `null`; before the resolver split it was gated on `continueId === null`, which (because the resolver always fell back to a non-locked question) was never true — so this completed UX was unreachable. See `docs/decisions/DECISIONS.md`.
...
- **Solution reveal placement**: on **code tracks** the "Review Official Solution" control lives in the verdict header once reveal criteria are met. On **MCQ tracks** that header toggle is suppressed — the reveal lives in the reasoning ladder ("Reveal the answer"), which renders the explanation **in place** and lights the correct option tile (see [`practice-modality-spec.md`](specs/practice-modality-spec.md) § MCQ wrong-answer journey).
...
- **Progressive hints reveal**: hints are revealed one at a time in a stepwise card. Multi-hint questions now use difficulty-aware labels (`Mental model`, `Core pattern`, `Decomposition`, etc.) derived from the question difficulty and hint count, while single-hint questions render as a simple `Guiding hint` reveal so the UI does not imply a missing ladder. The soft gate now differs by family: **code tracks** keep "all hints exhausted before the solution shows"; **MCQ tracks** lead with the ladder but offer a quiet "Skip to the answer" escape hatch. On MCQ, a wrong answer is a **gated re-attempt** (not a retry loop or a dead end): the options lock until the answer is revealed, then re-open (`MCQPanel canReselect`) so the user selects the correct one and submits → solved, counting as a solve like any code-track solve; "Next" appears only on a solve and never re-opens the current question. Canonical: [`practice-modality-spec.md`](specs/practice-modality-spec.md) § MCQ wrong-answer journey.
...
- **Today's goal widget**: a small widget in the sidebar bottom tracks "questions solved today" against a user-set target (1–20, default 5, stored in `localStorage` under `session-goal`). The daily solve count is global across all tracks — it counts distinct practice questions the user newly solves today (first-time solves only; re-solves, sample questions, and mock questions do not count). Count is persisted in `localStorage` under `dt-daily-solves` as `{ date: "YYYY-MM-DD", count: N }` (local calendar day, resets at local midnight). A `'dt-daily-solves-changed'` CustomEvent is dispatched on every increment so all open tabs update live; the widget also refreshes on `storage`, `focus`, and `visibilitychange` events. Increment logic lives in `frontend/src/utils/dailyGoal.js`; the increment is called from `QuestionPage.handleSubmit` on a correct submission when the question was not already solved before this submit. Widget label is "Today"; reached message is "Goal reached — nice work today!".
...
SEO role landing page (auth-agnostic — same content logged in or out). Config-driven: reads `:role` from the URL, looks it up in `roleRegistry.js` (`ROLES`), and renders from a per-role `ROLE_CONTENT` map (hero, "what interviews test", per-role track cards, a shared reasoning section, FAQ). Unpublished roles (or unknown slugs) render `NotFoundPage`. Includes a 3-level breadcrumb (`datathink / Interview prep / <Role>`), Helmet title/description, and `BreadcrumbList` + `FAQPage` JSON-LD. Hero CTAs: "Try a free sample →" → `/sample?role=<slug>` (filtered hub), and "See the N tracks ↓" smooth-scrolls to the on-page track grid. Below-hero sections animate in via the shared `Reveal`. Track cards have no left accent strip (color lives in the dot + Practice link + hover-border); see `.ip-track-card` in `App.css`. Each track card also carries an **`emphasis`** line (rendered under an "Interview weight" label, `.ip-track-emphasis`) — a one-line read on how heavily that track is weighted *for that role* and the difficulty depth that is realistically sufficient (e.g. SQL is the core analyst screen, depth through hard; Python for a data scientist is supporting, medium mastery is enough). It is editorial copy in `ROLE_CONTENT`, not derived; it complements the role→track mapping (which stays the `roleRegistry.js` SoT) and the per-track difficulty framing in `docs/tracks/<track>.md`. The index page deliberately does **not** repeat the per-track weight (it is a role chooser; weight belongs on the role detail page).
...
**Shared pieces:** `frontend/src/roleRegistry.js` is the **single source of truth** for the role→track mapping + `slug` + `hasPage`. It is consumed directly by the landing role selector, the role pages, the index, and the Sample Hub `?role=` filter, and **derived from** — no second hardcoded mapping — by MockHub `MOCK_ROLES` (mock role selector, plus the Mixed benchmark question count + track-list summary via `mockModeConfig`, both = the role's `tracks.length`) and ProgressDashboard `ROLE_TRACK_FILTERS` (dashboard role filter). Adding a track to a role updates `ROLES` only; every other surface follows. Mirror of [`specs/platform-north-star.md`](specs/platform-north-star.md) §Role-to-track. `frontend/src/components/Reveal.js` is the shared scroll-reveal wrapper (`Reveal` + `useInView`, the `.lp-reveal` IntersectionObserver pattern extracted from the landing). The `Topbar` carries a **"By Role" dropdown** (the four roles → `/interview-prep/<slug>` + "All roles" → `/interview-prep`, mirroring the Practice dropdown) so role pages are reachable from every page — important since the logged-in landing has no role selector.
...
- `frontend/public/branding/linkedin-banner.svg` — 1128×191 LinkedIn Company Page cover; lockup + tagline only (no stat row — the format is too short at 191px to carry it cleanly). Composition is shifted right of true center (centered at x=700, not x=564) so it clears LinkedIn's bottom-left logo overlap. Source for `linkedin-banner.png`, rendered at 2x (see below).
- `frontend/public/branding/linkedin-banner-light.svg` — 1128×191 light-ground alternate of the LinkedIn banner. Source for `linkedin-banner-light.png`, rendered at 2x. Standalone, for light-background placements.
...
Social banners: `branding/x-banner.png` (1500×500, dark), `branding/x-banner-light.png` (1500×500, light), `branding/linkedin-banner.png` (2256×382 pixels — 1128×191 spec rendered at a `scale: 2` TARGETS field for retina sharpness, dark), `branding/linkedin-banner-light.png` (2256×382 pixels, same 2x treatment, light).

**Render script:** `frontend/scripts/render-brand-assets.mjs` — Node ESM, uses Playwright Chromium. Run from `frontend/`: `node scripts/render-brand-assets.mjs`. Each TARGETS entry carries a `colorScheme` field (`'dark'` or `'light'`); before each screenshot the script calls `page.emulateMedia({ colorScheme })` so the adaptive `favicon.svg` renders in the correct scheme. The OG font-load branch triggers on `source.startsWith('og-image')`, covering both `og-image.svg` and `og-image-light.svg`. It inlines each SVG directly into the DOM (required for OG web-font pickup), sets an exact-pixel viewport at the target's logical `width`/`height`, and screenshots with `deviceScaleFactor: 1` by default. An optional per-target `scale` field (used by the two `linkedin-banner*` targets, `scale: 2`) is passed as `deviceScaleFactor` on `browser.newPage()` instead, so the clip stays in logical CSS pixels but the output PNG is rasterized at `width*scale × height*scale` for retina sharpness.
...
| TierBanner | `components/TierBanner.js` | One-line banner at the top of TrackHubPage. Free users see "Free plan · Easy questions only. Unlock medium & hard:" + **Upgrade to Pro** (`UpgradeButton variant="link"` — Pro is the contextual unlock for medium/hard; opens checkout in place) and **See plans →** (routes to `/pricing` with `returnTo`, full Pro/Elite comparison) — both rendered as subtle text links so the banner stays quiet and doesn't compete with the primary "Continue" CTA. Pro/Elite see their plan label. No thresholds or solve-count logic. Takes a `returnTo={{ path, label }}` prop from TrackHubPage. |
| UpgradeButton | `components/UpgradeButton.js` | Reusable upgrade CTA. Picks the billing rail via `railForCurrency` (`utils/currency.js`) from the **silently detected** currency (`detectCurrency()` — India `Asia/Kolkata`/`Asia/Calcutta` → INR, else → USD): INR → Razorpay Checkout, any other currency → Paddle.js overlay (Merchant of Record). There is **no user-facing currency selector** (removed); every upgrade surface is rail-consistent without the caller threading a currency value. `variant` prop: `"button"` (default, filled `btn-primary`) or `"link"` (subtle text link via `.upgrade-link`, for low-key surfaces like the TierBanner). Logged-out clicks route to `/auth` (carrying `from` + `upgradeTier`); a create-order failure shows the backend's error message inline (`upgrade-btn-error`). `autoTrigger` prop (ref-guarded against double-invoke): opens checkout for `tier` on mount once `user` is set, instead of waiting for a click — used by `LandingPage` to resume an upgrade after sign-in. |
...
- **Resume entry (`?resume=1`)**: when present on a hub URL (`/practice/:topic?resume=1`, the landing "Resume" card's target), AppShell waits for the catalog, then resolves the track's **next-up** question via `pickNextUpQuestionId` (`utils/catalogNav.js`) — the backend `is_next` flag, scanning easy→hard. If a next-up exists it redirects there (`replace`, param stripped); **if there is none (the track is fully solved / all accessible questions done), it strips `?resume=` and stays on the hub** rather than re-opening a solved question — the hub then shows its completed state (Explore tracks / Unlock more, see TrackHubPage §Completed state). Mirrors the `?drill=`/`?concepts=` hub-redirect effects. `pickNextUpQuestionId` is `is_next`-only and **nullable** (the resolver split from `pickContinueQuestionId = pickNextUpQuestionId ?? pickFirstQuestionId`, which the hub's Continue/Start button uses); making "no next-up" representable as `null` is what lets both the resume redirect and the hub's `allAccessibleSolved` state work — see `docs/decisions/DECISIONS.md`.
- **Today's goal widget**: shown in the sidebar for logged-in users. Tracks distinct practice questions newly solved today (global across all tracks, resets at local midnight). Goal is adjustable (1–20), stored in `localStorage` under `session-goal`. Daily count stored in `localStorage` under `dt-daily-solves`. See AppShell §Today's goal widget for full contract.
- Upgrade panel shown for `free` and `pro` plan users; lives in the sidebar beneath the question list (free users see "Easy questions are free — upgrade for all medium and hard")

### SidebarNav

Accepts a `plan` prop (passed from AppShell). On the free plan, easy questions are unlocked and medium/hard are locked (Pro/Elite unlock everything):
...
- **Locked rows**: on the free plan, medium and hard questions render as locked (the free tier covers easy only). Medium and hard require Pro/Elite — there is no per-track unlock ladder.
...
- MockHub hero frames `/mock` as the **Benchmark → Drill → Interview Loop** arc — the kicker and subtitle sequence the three modes, positioning the Loop as "the real-interview test once you're ready" rather than lumping it with drills. The "How it works" modal adds an explicit recommended-path step ending "a recommendation, not a gate — start any mode directly." The help button is a separate adjacent control. Canonical: [`docs/features/mock.md`](features/mock.md) § Interview Loop positioning.
- **Loop capstone soft-nudge:** when an Elite user selects Interview Loop on a track they have **not** benchmarked (and the rail isn't in a blocked/completion state), the session-brief rail shows a calm `Suggested first` nudge ("Run a quick {track} benchmark first … You can start it now either way"). Driven by the `loopTrackBenchmarked` flag; framing only — it never disables Start.
...
- Benchmark summaries show footer actions: `Share result` + `Back to Mock` + `Plan follow-up drill` (primary). **Loop capstone:** after a *strong* single-track benchmark (≥60%) by an Elite user whose holistic track readiness score is ≥65 (`showLoopCapstone`), the footer promotes `Try an Interview Loop →` to primary and demotes `Plan follow-up drill` to secondary — the just-completed benchmark is the readiness evidence. See [`docs/features/mock.md`](features/mock.md) § Interview Loop positioning.
