...
- Phase 4 is in progress: mock backend now accepts a real `benchmark` mode with track-specific fixed shapes; Statistics benchmark enforces a `1 numerical + 2 conceptual` mix; MockHub now defaults to benchmark, reframes short/custom sessions as drills, and treats Mixed as drill-only; MockSession active and summary chrome now carry benchmark-vs-drill framing so the session experience matches the new setup model
