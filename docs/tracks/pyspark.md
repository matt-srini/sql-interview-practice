...
- **DPP mental model — broadcast is NOT a prerequisite, it is an optimization-reuse decision.** DPP fundamentally works by injecting a runtime subquery filter (`partition_col IN (SELECT join_key FROM filtered_dim)`) at the fact table's physical partition scan. The hard prerequisites are: (a) the fact table is physically partitioned on the join key, and (b) the join key matches the partition column. Broadcast involvement is a *deployment* detail controlled by `spark.sql.optimizer.dynamicPartitionPruning.reuseBroadcastOnly` (default `true`): when this is true, Spark only fires DPP if it can reuse the key set from an already-planned broadcast hash join, avoiding an extra dimension scan as an optimization; when false, DPP fires via an independent subquery scan even without a broadcast. **Every DPP question must teach broadcast as a reuse optimization, never as a prerequisite of the DPP mechanism itself.** Phrasings like "DPP requires a broadcast" or "broadcast is a prerequisite for DPP" are forbidden — they train the wrong optimizer mental model. Correct phrasings: "DPP reuses the broadcast's key set under the default `reuseBroadcastOnly=true`," "without a broadcast to reuse, DPP deactivates under the default config," "DPP can fire independently of broadcast by setting `reuseBroadcastOnly=false`." Stems may pin either default or non-default config; whichever is pinned, the explanation must use language consistent with this mental model.
- **DPP topic discipline — if broadcast/`reuseBroadcastOnly` is not the question's primary lesson, do not mention it.** Even mentioning broadcast as "not a prerequisite" in the explanation draws the learner's attention to broadcast as if it were related to the DPP mechanism. DPP questions about partition-key alignment, physical vs. in-memory partitioning, or other prerequisites should NOT include broadcast distractors or broadcast clarifications in their explanations — the topic is best left out entirely so it cannot reinforce the wrong mental model. The single exception is a question whose primary topic IS the `reuseBroadcastOnly` mechanism (e.g. AQE-swap deactivates DPP under default reuse mode); that question may discuss broadcast as the central teaching, but no other DPP question in the bank should.
