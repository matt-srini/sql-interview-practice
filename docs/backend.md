...
- **Sandbox env thread pinning:** `_sandbox_env` also exports `OPENBLAS_NUM_THREADS`/`OMP_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMEXPR_NUM_THREADS`=`1` so OpenBLAS's per-core scratch-buffer pool stays within the 512 MB `RLIMIT_AS` on many-core hosts (over-reported inside containers); without pinning the virtual reservation can exceed the cap before numpy even imports, aborting the process. Only the numpy-importing paths (Pandas, Statistics-numerical) are affected; single-threaded BLAS is also deterministic, matching the single-threaded DuckDB grading path.
- **Non-zero exit logging:** `_spawn_harness` emits a `logger.warning` when the subprocess exits non-zero — an infra-level harness failure (OOM / OpenBLAS abort / segfault / RLIMIT kill), as distinct from user-code errors (which are caught inside the harness and return exit 0 with an error payload). Previously these were returned as a normal HTTP 200 `{error}` body, invisible to Sentry.
