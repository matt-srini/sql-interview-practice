...
- **No stale docs, ever.** Every change to code, strategy, philosophy, or product behaviour MUST update the relevant source-of-truth docs in the same commit — not as a follow-up, not "later." Stale docs are the single largest cause of drift on this platform. The mapping below is comprehensive; use it.
  ...
  | **Sandbox threat model** (defense layers, what each contains/doesn't, residuals) | `docs/specs/sandbox-threat-model.md` (canonical) |
...
- **★ DELEGATION GATE — sizable mechanical edits go to Sonnet; Opus orchestrates, reviews, commits.** Before starting any implementation work, STOP and decide the tier: if the edits are *sizable or cleanly decomposable* — **rule of thumb: 3+ files, or a batch that splits into disjoint slices** — hand them to **Sonnet** subagent(s) (`Agent` tool, `model: sonnet`) with an exact deterministic spec (absolute paths, precise changes/values, what to remove, the tests + docs to touch, the verification commands, and the literal line *"do NOT run git — the orchestrator commits"*). **Parallelize disjoint files** across subagents; keep coupled edits (same file) in ONE session. **Opus never delegates** orchestration, design decisions, diff review, or commit/land — and **may make trivial edits inline** (a typo, a one-liner, the review/integration touch-ups, config/docs/decision entries). Rationale is cost + fit: Sonnet is the right tier for well-specified mechanical edits; Opus is reserved for judgment. This is the rule's canonical home; a first-implementation-edit nudge hook (`.claude/settings.local.json`) only re-surfaces it at the edit moment to fight start-of-session context-decay (the reason it was historically ignored). Full handoff procedure: the `delegate_to_sonnet` memory.
...
- **Every push to `origin/main` must leave CI green — verify it, never assume it.** After any push to `origin/main` (via `scripts/land.sh` or a direct commit + push), confirm the resulting GitHub Actions run passes before the task is considered done: `gh run list --commit "$(git rev-parse HEAD)"`, then `gh run watch <run-id>` (or poll until `completed`). CI is the gate that the live product still builds and the full test + content-validation suite still passes — a red `main` means the next deploy ships broken. **If CI is red, fix it in the same session — even if the failure is pre-existing or not caused by your change.** A red pipeline you inherited is still a red pipeline you must leave green; never push more work on top of a red `main` and walk away. The invariant, alongside `main` ≡ `origin/main`: **you never end a task on a red `origin/main` CI run.** (CI lives in `.github/workflows/ci.yml`: `backend-tests` runs migrations + `scripts/validate_content.py` + `pytest`; `frontend-build` runs `npm run lint` (ESLint `react-hooks/rules-of-hooks`, error-level — recurrence guard for the SchemaViewer "Rendered fewer hooks than expected" crash) + `npm test` + `npm run build`. Note `backend-tests` does **not** build the frontend, so any test that needs the SPA's `index.html` must provide its own — the recurrence guard is the `_frontend_index` fixture in `backend/tests/test_seo_phase1.py`.)
...
- **Mock mode** — 1,169 additional mock-only questions (Pro/Elite), never shown in practice catalog
...
**Tracks:**
- **SQL** — 118 practice (37 easy / 50 medium / 31 hard) + 196 mock-only, DuckDB execution, realistic relational datasets
...
|---|---|---|---|---|---|
| SQL | 37 + 0 | 50 + 67 | 31 + 129 | SQL query via DuckDB | `backend/content/questions/` |
...
**Mock-only totals:** SQL 196 · Python 103 · Pandas 132 · PySpark 150 · Statistics 134 · ML Fundamentals 143 · Experimentation 104 · Data Modeling 97 · Data Engineering 110 = **1,169 mock-only questions** (Pro/Elite only)
...
**Role interview-prep pages (SEO):** `/interview-prep` (roles index) + `/interview-prep/:role` (`data-engineer` | `data-analyst` | `analytics-engineer` | `data-scientist`) — auth-agnostic role landing pages. **`frontend/src/roleRegistry.js` is the single source of truth for the role→track mapping** (id, slug, hasPage, tracks; mirror of [`docs/specs/platform-north-star.md`](docs/specs/platform-north-star.md) §Role-to-track), consumed by the landing role selector, the role pages, the index, and the Sample Hub `?role=` filter, and **derived by** the mock role selector (`MockHub` `MOCK_ROLES`, including the Mixed benchmark question count via `mockModeConfig` = the role's `tracks.length`) and the dashboard role filter (`ProgressDashboard` `ROLE_TRACK_FILTERS`) — no surface restates the mapping or the count. See [`docs/seo.md`](docs/seo.md) §Roadmap (Phase 2) + [`docs/frontend.md`](docs/frontend.md).
...
- **Active theme: Forest & Ink — light-only at launch.** Dark mode is **deferred to a future version** and sits **dormant**: the `[data-theme="dark"]` CSS (App.css, incl. the brand-mark `.brand-mark-primary/secondary` fill overrides) and the CodeEditor forest/charcoal switch remain in the codebase but are unreachable — `ThemeProvider` (App.js) and the `index.html` pre-paint bootstrap both lock `theme='light'` / `isDark=false`, ignoring `localStorage` + OS `prefers-color-scheme` (and flushing any stored pref), and the toggle UI is removed from `Topbar`. Re-enabling dark is a near-one-line flip in both places. Two-tone editor (always dark): forest-dark under light pages (the charcoal `#16181C` dark-page variant is dormant). See `docs/decisions/DECISIONS.md` (2026-06-17 defer-dark) + `docs/design/color-palette.md` § Code editor & sandbox surfaces.
- **The brand mark is inline SVG (`frontend/src/components/BrandMark.js`), never a fetched `<img>`.** It renders the two-rect monogram inline (light fills baked in as attributes; dark via the dormant CSS above), so it makes **zero network requests** and cannot "go missing" on a server blip / moved file / SPA-fallback mis-serve — the recurring prod-blocker the old `<img src="/branding/mark-no-bg.svg">` (+ `isDark` src switch) kept causing. Use `<BrandMark />` for the in-app logo everywhere (Topbar, sample topbars); the `/branding/mark-*.svg` files are artwork sources for favicon/OG regen only, not loaded by the app. Regression guard: `BrandMark.test.js` asserts inline-svg-not-`<img>`. See `docs/decisions/DECISIONS.md` (2026-06-21 inline-brand-mark) + `docs/design/color-palette.md` § Logo / brand mark.
...
**Sandbox security layers (as of 2026-06-26).** Canonical threat model (every layer + residuals): [`docs/specs/sandbox-threat-model.md`](docs/specs/sandbox-threat-model.md). Summary of the layers:
1. **AST guard** (`python_guard.py`) — blocks dangerous bare names (`globals`, `locals`, `getattr`, `eval`, `exec`, `__builtins__`, `__import__`, etc. via `visit_Name`), dunder chains (`__class__`, `__globals__`, `__subclasses__`, frame/traceback walks via `visit_Attribute`), **`str.format`/`.format_map` templates that hide a dunder accessor inside the string literal (`"{0.__globals__}".format(solve)` — the field spec isn't an AST node, so it's caught by `visit_Constant` scanning replacement-field names for `__`)**, all imports except the per-track allowlist, and pandas/numpy filesystem methods. **Defense-in-depth (2026-06-24):** the subprocess harness also runs user code with a **restricted `__builtins__`** (`python_sandbox_harness._safe_builtins` — the dangerous builtins the guard blocks are removed, keeping `__import__`/`__build_class__` that `import`/`class` need at runtime), so the AST guard is not the *sole* gate against builtins. Red-teamed in `tests/test_guard_redteam.py` (40 escape attempts all BLOCKED, incl. the format-string class, 13 legit snippets PASS). The guard also scans string/bytes literals for escape-gadget dunders and blocks module re-export attribute hops (`_os`/`_sys`/`sys`/`os`/`modules`), closing the runtime-string + re-export escapes found in the 2026-06-26 audit (`operator.attrgetter('__globals__')`, `random._os`, `typing.sys.modules`).
2. **Scrubbed subprocess env** (`python_evaluator._sandbox_env`) — the harness subprocess sees only `PATH/HOME/LANG/locale/TMPDIR/TZ` + Python encoding flags. Every production secret (`DATABASE_URL`, `RAZORPAY_KEY_SECRET`, OAuth secrets, `RESEND_API_KEY`, `SENTRY_DSN`) is absent. No `PYTHONPATH`, so user code cannot import backend modules. Tested in `tests/test_sandbox_env_isolation.py`. It also pins native-math thread pools (`OPENBLAS_NUM_THREADS`/`OMP_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMEXPR_NUM_THREADS`=1) so OpenBLAS's per-core buffer reservation stays under the 512 MB `RLIMIT_AS` on a many-core host (the alternative is an OpenBLAS abort on the Pandas / Statistics-numerical paths). The harness also guards the runtime `__import__` to deny code-exec/native/IPC/deser modules (`subprocess`/`ctypes`/`multiprocessing`/`pickle`/…), and installs a second seccomp filter denying `execve`/`execveat` so a guard escape cannot spawn a subprocess (`tests/test_sandbox_runtime_hardening.py`).
...
6. **Resource caps (category-3 / DoS)** — per-subprocess `RLIMIT_AS` 512 MB (memory bomb), `RLIMIT_NPROC` 256 (fork bomb), `RLIMIT_FSIZE` 64 MB (disk fill), `RLIMIT_CPU` 14 s (backstop). Wall-clock timeouts 5 s/12 s enforced via `Popen` + **process-group SIGKILL** (`_kill_process_group` → `os.killpg`), so a forked child cannot outlive the timeout (plain `subprocess.run` only kills the direct child). Output caps (64 KB stdout, 512 KB result). **All code execution runs off the single event loop** (`backend/offload.py`): subprocess sandboxes via `asyncio.to_thread` under the global `MAX_CONCURRENT_EXECUTIONS` semaphore (default **cores − 2**, where "cores" is a cgroup-aware `effective_cpu_count()` in `backend/config.py` — not `os.cpu_count()`, which reports the host core count inside a container and would inflate the cap and peak sandbox memory), and DuckDB SQL serialized behind a process-wide lock (single in-process engine; concurrent connection use segfaults). Applied uniformly across every code-exec path (practice SQL/Python/Pandas, statistics, sample, mock) — so a slow execution never freezes other users (head-of-line blocking, measured + fixed: see `docs/decisions/DECISIONS.md` 2026-06-08 offload entry and `backend/loadtest/`) and the semaphore now genuinely bounds concurrency + peak sandbox memory. Container RAM cap = the one remaining Railway-dashboard item (Settings → Scale → **Replica Limits**; size above app + semaphore×512 MB). Tests: `tests/test_sandbox_resource_limits.py` (infinite loop, memory bomb, fork-bomb killpg, output flood, recursion) + `tests/test_concurrency_smoke.py` (off-loop + bounded + SQL-serialized). See `docs/deployment.md` § Sandbox security hardening and § Concurrency & scaling model.
...
**Learning paths and unlocks:** Paths are curated walks through the practice catalog and **do not unlock questions**. All paths are accessible to all users (free/anonymous included) — within a path, the standard plan policy applies: free users see easy questions unlocked and medium/hard locked. A user who solves a path question gets the same `solved` state as solving from practice directly. See `docs/content-authoring.md` §Paths for the canonical path model (patterns, roles, validator integrity rules).
