...
- **Curriculum designer** — The 852 practice questions have intentional difficulty progressions, real-world datasets with deliberate edge cases, and semantic concept tags. Changes to unlock rules, question ordering, or content must preserve the learning arc. Don't make hard questions trivially accessible or easy questions feel insulting.
...
- **Audits benchmark against durable contract docs, never the tracker.** The Phase 2 tracker self-deletes; durable rules live in `docs/content-authoring.md` (cross-track contract), `docs/tracks/<track>.md` (per-track contract), `docs/concept-taxonomy.md` (family registry + blocklist + dimensions), `.github/agents/question-authoring.agent.md` (procedure + checklist), `docs/features/mock.md` (mock contract), and `backend/concept_families.py` / `backend/scripts/validate_content.py` (machine enforcement). A post-execution audit verifies questions against THOSE — the tracker brief is consulted only for one-time migration items (remap residuals, deprecation lists, sizing-this-round).
...
**Modes per track:**
- **Challenge mode** — plan-aware unlock rules, persistent progress, 852 practice questions across 9 tracks
- **Mock mode** — 523 additional mock-only questions (Pro/Elite), never shown in practice catalog
...
- **Pandas** — 86 practice (27 easy / 36 medium / 23 hard) + 110 mock-only, pandas-specific data manipulation, DataFrame comparison
- **PySpark** — 128 practice (41 easy / 45 medium / 42 hard) + 150 mock-only, MCQ / predict-output / debug / scenario formats
...
- **Data Modeling** — 76 practice (25 easy / 28 medium / 23 hard) + 13 mock-only, MCQ / scenario, no code execution; `eval_kind="mcq"`, `unlock_profile="mcq"`, `in_mixed_mock=false`
...
- **ML Fundamentals** — 96 practice (30 easy / 38 medium / 28 hard) + 25 mock-only, MCQ / scenario / predict-output / debug, no code execution; `eval_kind="mcq"`, `unlock_profile="mcq"`, `in_mixed_mock=false`
- **Experimentation** — 84 practice (30 easy / 32 medium / 22 hard) + 25 mock-only, MCQ / scenario / predict-output / debug, no code execution; `eval_kind="mcq"`, `unlock_profile="mcq"`, `in_mixed_mock=false`
...
| Pandas | 27 + 0 | 36 + 50 | 23 + 60 | DataFrame function, output comparison | `backend/content/python_data_questions/` |
| PySpark | 41 + 0 | 45 + 75 | 42 + 75 | MCQ / predict-output / debug / scenario | `backend/content/pyspark_questions/` |
| Data Engineering | 30 + 0 | 35 + 6 | 26 + 8 | MCQ / scenario / debug | `backend/content/data_engineering_questions/` |
| Data Modeling | 25 + 0 | 28 + 6 | 23 + 7 | MCQ / scenario | `backend/content/data_modeling_questions/` |
...
**Practice totals:** SQL 115 · Python 79 · Pandas 86 · PySpark 128 · Data Engineering 91 · Data Modeling 76 · Statistics 97 · ML Fundamentals 96 · Experimentation 84 = **852 practice questions**  
**Mock-only totals:** SQL 162 · Python 100 · Pandas 110 · PySpark 150 · Statistics 8 · ML Fundamentals 25 · Experimentation 25 · Data Modeling 13 · Data Engineering 14 = **607 mock-only questions** (Pro/Elite only)
