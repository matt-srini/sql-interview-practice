...
- **Curriculum designer** — The 862 practice questions have intentional difficulty progressions, real-world datasets with deliberate edge cases, and semantic concept tags. Changes to unlock rules, question ordering, or content must preserve the learning arc. Don't make hard questions trivially accessible or easy questions feel insulting.
...
- **Active refactor — the canonical pickup point (transitional).** When in doubt about "what's the next work" during the 2026-05 authoring refactor, read [`docs/phases/2026-05-authoring-refactor.md`](docs/phases/2026-05-authoring-refactor.md) — currently the single self-contained tracking doc for the in-flight authoring-system refactor. It explains every locked decision with rationale and contains the execution log + decision rows for all closeouts. **Status:** all 9 Phase 2 rounds + both Phase 2.5 cycles closed 2026-05-26; Phase 3 (Interview Loop + Mixed benchmark + mock consolidation) is the remaining work. **Once Phase 3 ships, do all of the following in the same commit:** (1) move `docs/phases/2026-05-authoring-refactor.md` → `docs/archive/2026-05-authoring-refactor.md`; (2) delete this CLAUDE.md bullet; (3) grep every file under `docs/` for the strings `2026-05-authoring-refactor`, `Phase 2 authoring refactor`, `Phase 3 mock`, `authoring refactor`, `short_drill`, `custom_drill` and remove or rewrite every occurrence — these are transitional terms with no place in durable docs; (4) update any cross-links in durable docs that pointed to the tracker to point to `docs/archive/2026-05-authoring-refactor.md` if historical context is genuinely needed, or remove the link entirely if not.

- **Phase 2 orchestration runbook — DURABLE counterpart to the tracker.** Any Opus session running orchestration for a future Phase 2 (new track added post-refactor), Phase 2.5 (re-balance or new-family addition), or Stage C audit must read [`docs/orchestration-runbook.md`](docs/orchestration-runbook.md) before producing any Stage artefact. All active tracks closed Phase 2 by 2026-05-26 (the audience header on the runbook reflects this). The runbook codifies the three-stage process (A → B → C), the Stage A/B/C template skeletons, the retro-cleanup pattern, the historical Phase 2 status table, the precedent table, and the lessons-learned (chain children IN locked total per ML Stage C; pattern-shadow check per Stats Stage C; new-family reasoning-depth defence per ML BIAS/FAIRNESS Phase 2.5). Unlike the tracker bullet above, this doc is **durable** — it survives Phase 3 and the tracker archival, and is reusable for any future track Phase 2.
...
**Modes per track:**
- **Challenge mode** — plan-aware unlock rules, persistent progress, 865 practice questions across 9 tracks
