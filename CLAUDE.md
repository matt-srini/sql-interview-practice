...
- **Learning paths:** 46 total — SQL: 9, Python: 6, Pandas: 5, PySpark: 5, Data Engineering: 3, Data Modeling: 5, Statistics: 3, ML Fundamentals: 5, Experimentation: 5. Each track has exactly one `foundational` path (UX entry point); intermediate and advanced are uncapped. Paths are curated 5–9 question walks through a *pattern* (practitioner skill — see `docs/content-authoring.md` §Paths). Paths do not unlock anything; unlocks follow the standard practice thresholds.
...
6. **Resource caps (category-3 / DoS)** — per-subprocess `RLIMIT_AS` 512 MB (memory bomb), `RLIMIT_NPROC` 256 (fork bomb), `RLIMIT_FSIZE` 64 MB (disk fill), `RLIMIT_CPU` 14 s (backstop). Wall-clock timeouts 5 s/12 s enforced via `Popen` + **process-group SIGKILL** (`_kill_process_group` → `os.killpg`), so a forked child cannot outlive the timeout (plain `subprocess.run` only kills the direct child). Output caps (64 KB stdout, 512 KB result). Global asyncio concurrency semaphore defaults to **cores − 2** (`MAX_CONCURRENT_EXECUTIONS`) — leaves CPU headroom for the app and bounds peak sandbox memory. Container RAM cap = the one remaining Railway-dashboard item (Settings → Scale → **Replica Limits**; size above app + semaphore×512 MB). Tests: `tests/test_sandbox_resource_limits.py` (infinite loop, memory bomb, fork-bomb killpg, output flood, recursion). See `docs/deployment.md` § Sandbox security hardening.
