"""Python (algorithms) question catalog loaded from JSON content files."""

import json
from pathlib import Path
from typing import Any, Optional

_CONTENT_DIR = Path(__file__).resolve().parent / "content" / "python_questions"
_SCHEMA_CONFIG_PATH = _CONTENT_DIR / "schemas.json"


def _fail(question_id: int, reason: str) -> None:
    raise ValueError(f"Invalid python question (id={int(question_id)}): {reason}")
...
def _validate_question(question: dict[str, Any], *, id_ranges: dict[str, list[int]]) -> None:
    ...
    required = ["id", "order", "title", "difficulty", "description", "starter_code", "expected_code", "test_cases"]
