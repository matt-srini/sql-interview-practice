"""Experimentation question catalog loaded from JSON content files."""
...
def _fail(question_id: int, reason: str) -> None:
    raise ValueError(f"Invalid experimentation question (id={int(question_id)}): {reason}")
...
def _validate_question(question: dict[str, Any], *, id_ranges: dict[str, list[int]]) -> None:
    ...
    for f in required:
        if f not in question:
            _fail(qid, f"Missing required field: {f}")
...
def get_public_question(question: dict[str, Any]) -> dict[str, Any]:
    """Return question payload safe to send before answer is submitted."""
    return {
        "id": question["id"],
