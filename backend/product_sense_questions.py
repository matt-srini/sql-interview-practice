"""Product Sense question catalog loaded from JSON content files.

Constructed-reasoning track (eval_kind="mcq"): the candidate reads a product
scenario and commits to a single best answer. No code execution.

Question `type` values are deliberately limited to four (see
docs/tracks/product-sense.md §Modality): ``scenario`` (dominant), ``conceptual``,
``debug``, ``predict_output``. ``optimization`` and ``numerical`` are EXCLUDED —
there is no compute to optimise and nothing numerical to execute. The loader's
VALID_TYPES enforces that exclusion at import time.
"""
...
_CONTENT_DIR = Path(__file__).resolve().parent / "content" / "product_sense_questions"
_SCHEMA_CONFIG_PATH = _CONTENT_DIR / "schemas.json"

VALID_TYPES = {"conceptual", "predict_output", "debug", "scenario"}


def _fail(question_id: int, reason: str) -> None:
    raise ValueError(f"Invalid product_sense question (id={int(question_id)}): {reason}")
...
def _validate_question(question: dict[str, Any], *, id_ranges: dict[str, list[int]]) -> None:
    ...
    try:
        resolve_interaction_mode("product-sense", question)
...
def _load_questions() -> list[dict[str, Any]]:
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        for question in content:
            ...
            seen_ids.add(qid)
            questions.append({**question, "interaction_mode": resolve_interaction_mode("product-sense", question)})
