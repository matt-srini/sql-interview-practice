"""Data Modeling question catalog loaded from JSON content files."""
...
def _fail(question_id: int, reason: str) -> None:
    raise ValueError(f"Invalid data_modeling question (id={int(question_id)}): {reason}")
...
def _validate_question(question: dict[str, Any], *, id_ranges: dict[str, list[int]]) -> None:
    ...
    try:
        resolve_interaction_mode("data-modeling", question)
...
def _load_questions() -> list[dict[str, Any]]:
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        for question in content:
            ...
            seen_ids.add(qid)
            questions.append({**question, "interaction_mode": resolve_interaction_mode("data-modeling", question)})
