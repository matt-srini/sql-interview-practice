...
import experimentation_questions as catalog
...
router = APIRouter(prefix="/api/experimentation")

logger = logging.getLogger(__name__)


async def _get_unlock_state(
    current_user: dict[str, Any],
) -> tuple[dict[int, str], dict[str, int | None]]:
    grouped = catalog.get_questions_by_difficulty()
    solved_ids = await get_solved_ids(current_user["id"], topic="experimentation")
    unlock_state = compute_unlock_state(current_user["plan"], solved_ids, grouped, track="experimentation")
...
async def get_question_detail(
    ...
    unlock_state, next_questions = await _get_unlock_state(current_user)
...
async def submit_answer(
    ...
    logger.info(
        "[request_id=%s] /api/experimentation/submit: user_id=%s question_id=%s",
        request_id,
        current_user["id"],
        body.question_id,
    )
    ...
    unlock_state, _ = await _get_unlock_state(current_user)
    ...
    if correct:
        await mark_solved(current_user["id"], int(q["id"]), topic="experimentation")
