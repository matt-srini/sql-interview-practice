...
import product_sense_questions as catalog
...
router = APIRouter(prefix="/api/product-sense")

logger = logging.getLogger(__name__)


async def _get_ps_unlock_state(
    current_user: dict[str, Any],
) -> tuple[dict[int, str], dict[str, int | None]]:
    grouped = catalog.get_questions_by_difficulty()
    solved_ids = await get_solved_ids(current_user["id"], topic="product-sense")
    unlock_state = compute_unlock_state(current_user["plan"], solved_ids, grouped, track="product-sense")
    next_questions = get_next_questions(unlock_state, grouped)
    return unlock_state, next_questions


@router.get("/catalog")
async def get_ps_catalog(
...
async def get_ps_question_detail(
    question_id: int,
    current_user: dict[str, Any] = Depends(get_current_user),
) -> dict[str, Any]:
    ...
    unlock_state, next_questions = await _get_ps_unlock_state(current_user)
...
@router.post("/submit")
async def submit_ps_answer(
    body: PySparkSubmitRequest,
    current_user: dict[str, Any] = Depends(get_current_user),
) -> dict[str, Any]:
    request_id = get_request_id()
    logger.info(
        "[request_id=%s] /api/product-sense/submit: user_id=%s question_id=%s",
        request_id,
        current_user["id"],
        body.question_id,
    )
    ...
    unlock_state, _ = await _get_ps_unlock_state(current_user)
    ...
    if correct:
        await mark_solved(current_user["id"], int(q["id"]), topic="product-sense")
