...
This file is the canonical context reference for Codex in this repository. **Keep it current.**
...
| Product overview, tech stack, content footprint | This file (`AGENTS.md`) |
...
### Always commit
End every session of edits with a `git commit` carrying a specific message. Co-author: `Co-Authored-By: Codex Sonnet 4.6 <noreply@anthropic.com>`.
...
For detailed product behaviour, API reference, component docs, and design tokens — use these canonical docs rather than asking Codex to recall from memory:
