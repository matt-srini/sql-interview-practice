...
function AppleIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
      <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.7 9.05 7.43c1.38.07 2.33.74 3.14.8 1.19-.24 2.33-.93 3.6-.84 1.54.12 2.7.71 3.45 1.8-3.14 1.87-2.38 5.98.43 7.16-.57 1.56-1.32 3.09-2.62 3.93zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z" />
    </svg>
  );
}
...
export default function AuthPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === 'signin') {
        ...
        setStatus('success');
        setInfo('Password reset is not yet available. Please sign in with email and password.');
        return;
      }
    } catch (err) {
      setStatus('idle');
  ...
  async function handleOAuth(provider) {
    setError(null);
    try {
      await api.get(`/auth/oauth/${provider}`);
    } catch (err) {
      const msg = err?.response?.data?.error;
      setError(msg || `${provider} sign-in is not yet available.`);
    }
  }
